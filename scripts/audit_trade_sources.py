...
def main() -> None:
    parser = argparse.ArgumentParser(description="Diagnostics-only audit of trade/import-export source candidates.")
    parser.add_argument("--output-dir", default="diagnostics/trade_source_audit")
    parser.add_argument(
        "--live-probe",
        action="store_true",
        help="Reserved for a later bounded HTTP probe. Phase 6.5A uses the static registry only.",
    )
    ...
    print(f"hs_code_mappings={len(audit.hs_code_mappings)}")
