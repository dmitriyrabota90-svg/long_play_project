...
def main() -> None:
    parser = argparse.ArgumentParser(description="Diagnostics-only audit of weather source and region candidates.")
    parser.add_argument("--output-dir", default="diagnostics/weather_source_audit")
    parser.add_argument(
        "--live-probe",
        action="store_true",
        help="Reserved for a later bounded HTTP probe. Phase 6.3A uses the static registry only.",
    )
    ...
    print(f"recommended_regions={len(audit.recommended_regions)}")
