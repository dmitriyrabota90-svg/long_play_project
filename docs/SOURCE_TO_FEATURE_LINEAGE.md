...
| USDA PSD / FAS PSD Online | `usda_psd` via `scripts/run_collector.py` | `raw_responses`, `data/raw/usda_psd/...` | `supply_demand_observations`, `supply_demand_commodities`, `product_supply_demand_weights` | `daily_supply_demand_features`, then `daily_product_features` | supply_demand group: production, use, crush, imports/exports, stocks, stock-to-use, area/yield, revisions, `supply_demand_as_of_date`, `supply_demand_missing_flags` | controlled/manual collector; `supply_demand_daily` local builder | local-implemented-awaiting-server-batch for final daily/export integration |
| Daily feature builder | `python scripts/build_features.py daily` | reads normalized tables | `daily_product_features` | `daily_product_features` | identity, price, fx, calendar, energy, benchmarks, weather, news_events, trade, supply_demand groups | manual/scheduled for core daily features depending deployment | production-deployed for earlier groups; trade/supply-demand integration awaiting server batch |
...
| Supply-demand daily builder | `python scripts/build_features.py supply_demand_daily` | reads `supply_demand_observations`, `product_supply_demand_weights` | `daily_supply_demand_features` | `daily_product_features` after daily build | supply_demand group | local-only currently; server batch pending | local-implemented-awaiting-server-batch |
| Dataset export | `python scripts/export_dataset.py daily_features` | reads `daily_product_features`, `products` | file artifacts under `data/exports/` | CSV/Parquet plus JSON manifest | all documented export columns in `DATASET_DICTIONARY.md` | manual file export | local trade and supply-demand columns implemented; production export validation pending |
...
- Production server batch is still pending for the latest local trade and
